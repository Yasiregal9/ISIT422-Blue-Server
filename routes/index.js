...
router.get("/plantinfo/:id", function (req, res) {
  PlantInfo.find({}, (err, plantsInfo) => {
    if (err) {
      console.log(err);
      res.status(500).send(err);
    }
    let filteredPlant = plantsInfo.filter(plantInfo => plantInfo.id == req.params.id)
    res.status(200).json(filteredPlant[0]);
  });
...
router.post("/updateplant", function (req, res) {
  ...
  UserPlant.findOne(
    { plantID: updatePlant.plantID },
    function (err, userPlant) {
      if (err) {
        console.log(err);
      } else {
